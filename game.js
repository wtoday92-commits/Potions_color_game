...
(function(){
  ...
  function showSelectScreen(){
    ...
    const orders = tiers.map(t=> buildOrderDescriptor(pickConfigForTier(t, usedNames)));
    ...
    orders.forEach(ord=>{
      ...
      card.innerHTML = `
        ${focus ? `<div class="focus-badge">${visualHTML(FOCUS_ICONS[focus],'focus-img')} ${FOCUS_NAMES[focus]}</div>` : ''}
  ...
  function startOrder(ord){
    ...
    $('orderFocusTag').innerHTML = focus ? `${visualHTML(FOCUS_ICONS[focus],'focus-img')} фокус: ${FOCUS_NAMES[focus]}` : '';
    SFX.orderShow();
  ...
  function finalizeResult(scoreData, timeFrac){
    ...
    $('breakdown').innerHTML = components.map(c=>
      `<div class="row ${c.focused?'focused':''}"><span>${c.focused?visualHTML(FOCUS_ICONS[target.focus],'focus-img')+' ':''}${c.label}</span><span class="val">${Math.round(c.score*100)}%</span></div>`
    ).join('');
