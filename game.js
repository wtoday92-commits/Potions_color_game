...
(function(){
  ...
  function npcAvatarHTML(cfg){ return visualHTML(cfg.img || cfg.emoji, 'npc-img'); }
  ...
  function updateStickerTally(){
    $('stickerTally').innerHTML = `
      <span class="tally-item">${visualHTML(STICKERS.perfect,'tally-img')} <b>${stickerCounts.perfect}</b></span>
      <span class="tally-item">${visualHTML(STICKERS.good,'tally-img')} <b>${stickerCounts.good}</b></span>
      <span class="tally-item">${visualHTML(STICKERS.bad,'tally-img')} <b>${stickerCounts.bad}</b></span>
    `;
  }
  ...
  function drawJar(opts){
    ...
    const bubbleEls = pts.map((p,i)=>{
      ...
      return `
        ...
        <path d="${blobPath(p.x, p.y, r, seed+i*31)}" fill="rgba(125,255,106,.72)" stroke="rgba(220,255,210,.55)" stroke-width="1"/>
    ...
    svg.innerHTML = `
      ...
      <!-- tech cap: node + antenna -->
      <line x1="${cx}" y1="${topY-22}" x2="${cx}" y2="${topY-6}" stroke="#35e0ff" stroke-width="2.5"/>
      <circle cx="${cx}" cy="${topY-24}" r="3.5" fill="#ff4dd2"/>
      <rect x="${cx-16}" y="${topY-8}" width="32" height="24" rx="5" fill="#0d1430" stroke="#35e0ff" stroke-width="2"/>
      <line x1="${cx-9}" y1="${topY+2}" x2="${cx+9}" y2="${topY+2}" stroke="rgba(53,224,255,.6)" stroke-width="1.5"/>
      <!-- container body -->
      <path d="${bodyPath}" fill="rgba(53,224,255,.05)" stroke="#35e0ff" stroke-width="2.2"/>
      <g clip-path="url(#jarClip)">
        <rect x="${cx-w/2-6}" y="${topY+40}" width="${w+12}" height="${h}" fill="${fillRef}"/>
        <rect x="${cx-w/2-6}" y="${topY+40}" width="${w+12}" height="8" fill="rgba(255,255,255,.25)"/>
        ${bubbleEls}
      </g>
  ...
  function pickConfigForTier(tierNum){
    if(tierNum < 5) return DIFFICULTIES[tierNum-1];
    return pick([DIFFICULTIES[4], ...SPECIAL_ORDERS]);
  }
  ...
  function buildOrderDescriptor(cfg){
    ...
    const flavor = focus && cfg.ff ? pick(cfg.ff[focus]) : pick(cfg.flavors);
    return { cfg, focus, flavor };
  }
  ...
  function showSelectScreen(){
    ...
    const orders = tiers.map(t=> buildOrderDescriptor(pickConfigForTier(t)));
    const wrap = $('customerCards');
    wrap.innerHTML = '';
    orders.forEach(ord=>{
      const { cfg, focus, flavor } = ord;
      const card = document.createElement('div');
      card.className = 'customer-card';
      card.style.borderColor = TIER_COLORS[cfg.tier];
      card.style.color = TIER_COLORS[cfg.tier];
      card.innerHTML = `
        ${focus ? `<div class="focus-badge">${FOCUS_ICONS[focus]} ${FOCUS_NAMES[focus]}</div>` : ''}
        <div class="avatar">${npcAvatarHTML(cfg)}</div>
        <div class="info">
          <div class="npc-name">${cfg.name}</div>
          <div class="quote">«${flavor}»</div>
          <div class="reward">за идеал: +${Math.round(cfg.reward * (focus?1.25:1))}</div>
        </div>
      `;
  ...
  function startOrder(ord){
    ...
    $('orderAvatar').innerHTML = npcAvatarHTML(cfg);
    $('orderText').textContent = flavor;
    $('orderBubble').style.borderLeftColor = TIER_COLORS[cfg.tier];
    $('orderFocusTag').textContent = focus ? `${FOCUS_ICONS[focus]} фокус: ${FOCUS_NAMES[focus]}` : '';
    SFX.orderShow();
    ...
    $('phaseLabel').textContent = 'сканируй образец...';
  ...
  function startGuessPhase(){
    ...
    $('phaseLabel').textContent = 'воссоздай смесь по памяти!';
    ...
    if(flags.hasGradient){
      $('colorLabelA').textContent = 'Спектр А';
      ...
      $('colorLabelA').textContent = 'Спектр';
  ...
  function updatePlayerJar(){
    ...
    if(flags.hasShape){
      ...
      $('lblShape').textContent = SHAPE_NAMES[shapeIdx];
    }
  ...
  function computeScoreComponents(){
    ...
    if(flags.hasGradient){
      ...
      components = [
        {key:'color', label:'Спектр А', score:colorAScore, weight:0.225},
        {key:'colorB', label:'Спектр Б', score:colorBScore, weight:0.225},
        {key:'size', label:'Объём', score:sizeScore, weight:0.15},
        {key:'count', label:'Сгустки (кол-во)', score:countScore, weight:0.25},
        {key:'bsize', label:'Сгустки (размер)', score:bsizeScore, weight:0.15}
      ];
      ...
      components = [
        {key:'color', label:'Спектр', score:colorScore, weight:0.15},
        {key:'sat', label:'Накал', score:satScore, weight:0.1},
        {key:'shape', label:'Форма', score:shapeScore, weight:0.25},
        {key:'size', label:'Объём', score:sizeScore, weight:0.15},
        {key:'count', label:'Сгустки (кол-во)', score:countScore, weight:0.2},
        {key:'bsize', label:'Сгустки (размер)', score:bsizeScore, weight:0.15}
      ];
      ...
      const satScore = curveScore(1 - Math.abs(satFromIdx(S.sat.value) - target.sat)/70);
      components = [
        {key:'color', label:'Спектр', score:colorScore, weight:0.3},
        {key:'sat', label:'Накал', score:satScore, weight:0.15},
        {key:'size', label:'Объём', score:sizeScore, weight:0.15},
        {key:'count', label:'Сгустки (кол-во)', score:countScore, weight:0.2},
        {key:'bsize', label:'Сгустки (размер)', score:bsizeScore, weight:0.2}
      ];
    } else {
      const colorScore = curveScore(1 - hueDist(guess.hue, target.hue)/180);
      components = [
        {key:'color', label:'Спектр', score:colorScore, weight:0.35},
        {key:'size', label:'Объём', score:sizeScore, weight:0.2},
        {key:'count', label:'Сгустки (кол-во)', score:countScore, weight:0.25},
        {key:'bsize', label:'Сгустки (размер)', score:bsizeScore, weight:0.2}
      ];
    }
  ...
  function finalizeResult(scoreData, timeFrac){
    ...
    $('resultTitle').textContent = perfect ? 'Идеальная смесь!' : good ? 'Годная смесь' : 'Брак...';
    $('resultTitle').className = 'result-title ' + (good ? 'good' : 'bad');
    $('deltaVal').textContent = (delta>=0?'+':'') + delta;
    $('deltaVal').className = 'delta ' + (good ? 'good' : 'bad');
    $('speedNote').textContent = speedBonusPct >= 1 ? `⚡ бонус за скорость: +${speedBonusPct}%` : '';
    $('overallScore').textContent = overallPct + '%';
    $('breakdown').innerHTML = components.map(c=>
      `<div class="row ${c.focused?'focused':''}"><span>${c.focused?FOCUS_ICONS[target.focus]+' ':''}${c.label}</span><span class="val">${Math.round(c.score*100)}%</span></div>`
    ).join('');
  ...
  async function saveLeaderboardEntry(name, finalScore){
    ...
    list.push({ name: name || 'Аноним', score: finalScore, date: new Date().toLocaleDateString('ru-RU') });
    list.sort((a,b)=>b.score-a.score);
    const top = list.slice(0,50);
    if(FIREBASE_DB_URL){
      try{
        const obj = {};
        top.forEach((e,i)=>{ obj['e'+i] = e; });
        await fetch(FIREBASE_DB_URL + '/leaderboard.json', { method:'PUT', body: JSON.stringify(obj) });
      }catch(e){ localStorage.setItem(LOCAL_LB_KEY, JSON.stringify(top)); }
    } else {
      localStorage.setItem(LOCAL_LB_KEY, JSON.stringify(top));
    }
    return top;
  }
  function renderLeaderboard(list, highlightScore, elId){
    ...
    ).join('') || '<div style="color:var(--ink-dim);text-align:center;">Пока пусто — будь первым!</div>';
  }
