...
// ---------- NPCs & difficulty ----------
  ...
  // Патч (крышки): крышка — не отдельная "нашлёпка" поверх банки, а
  // продолжение того же контура, что и тело (см. capOutlineProfile() в
  // game.js) — points считаются в capT (0=шов с телом, 1=самый верх
  // крышки), а wf — МНОЖИТЕЛЬ от фактической ширины банки в шве (не
  // абсолютное число), поэтому любая крышка садится ровно на любую форму
  // банки без зазора. heightFrac — высота крышки как доля ширины банки w.
  // ridgeAt (опционально) — на этой доле capT рисуется тонкое кольцо-хайлайт.
  const CAP_NAMES = [
  ...
  const CAP_PROFILES = [
    { points:[[0,1],[0.12,1],[0.18,1.5],[0.85,1.5],[1,1.4]], smooth:false, heightFrac:0.42, ridgeAt:0.5 },
    { points:[[0,1],[0.25,1.3],[0.55,1.35],[0.8,1.0],[1,0.15]], smooth:true, heightFrac:0.5 },
    { points:[[0,1],[0.3,1],[0.4,1.85],[0.5,1.85],[0.6,1],[1,1]], smooth:false, heightFrac:0.4, ridgeAt:0.45, spokesAt:0.45 },
    { points:[[0,1],[0.5,0.85],[0.85,0.7],[1,0.65]], smooth:true, heightFrac:0.55, ridgeAt:0.35 },
    { points:[[0,1],[0.3,1.15],[0.6,0.6],[1,0.05]], smooth:false, heightFrac:0.6 },
    { points:[[0,1],[0.2,1.25],[0.8,1.25],[1,1.1]], smooth:true, heightFrac:0.38, ridgeAt:0.5 },
    { points:[[0,1],[0.2,1.2],[0.4,0.95],[0.6,1.15],[0.8,0.85],[1,0.5]], smooth:true, heightFrac:0.35 },
    { points:[[0,1],[0.3,0.9],[0.7,0.9],[1,0.75]], smooth:false, heightFrac:0.3, antenna:true }
  ];
