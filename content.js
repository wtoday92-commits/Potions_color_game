...
// ---------- NPCs & difficulty ----------
  ...
  // (ширина у всех кусков одна и та же — artW). Вокруг самого стекла в
  // холсте оставлены прозрачные поля (место для неонового свечения) —
  // contentW/contentX0 говорят, где на холсте реально начинается и сколько
  // весит СТЕКЛО (не сам холст), чтобы масштаб считался от него, а не от
  // полного PNG — иначе жидкость (которая всегда ровно шириной w) вылезает
  // за нарисованные края банки.
  const CUSTOM_BOTTLES = [
    {
      id: 'b1', artW: 768, contentW: 530, contentX0: 118,
